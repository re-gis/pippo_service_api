...
export class PropertyService {
  ...
  originalCompsRating: number = 0;
  originalCompsSelected: Array<any>;
  originalComps: Array<any>;
  propertyCompalatorValue: number = 0;
  ...
  simpleQuality: boolean = true;
  cmaAveragePer: number = 0;
  ...
  constructor(private _http: Http, ngZone: NgZone, authService: AuthService) {
    ...
    this.propertyFlipData = {
        ...
        roi: 0,
        roiSliderPercent: 30,
        repairRemodelType: "rrHit",
        repairRemodelCustomTotal: 0,
        repairAndRemodel: {
            exterior: {
              paintOptions: [],
              windowOptions: [],
  ...
  parsePropertyId(idString: string) {
    ...
    for (let x of addr) {
      ...
      if (-1 !== ['obpropid', 'mlsnumber', 'as', 'tr'].indexOf(String(splitAddr[0]))) {
        parsedAddr.push(x);
      }
    }
  ...
  getPropertyFlags() {
    if (this.property && this.property.flags) {
      return this.property.flags;
    }

    return [];
  }

  getPropertyFlag(flag: string) {
    flag = flag.toLowerCase();

    if (this.property && this.property.flags) {
      for (let o of this.property.flags) {
        if (o.flag.toLowerCase() == flag) {
          return o;
        }
      }
    }
  ...
  indexReconciledRemarks(tax, mls, out, pre = '') {
    for (let k in tax) {
      if (tax.hasOwnProperty(k) && mls.hasOwnProperty(k)) {
        if (typeof tax[k] !== typeof mls[k]) {
          continue;
        } else switch (typeof tax[k]) {
          case "object":
            if (!tax[k] || !mls[k]) {
              continue;
            }

            this.indexReconciledRemarks(tax[k], mls[k], out, pre + '.' + k);
            continue;
          case "undefined":
            continue;
          case "string":
            if (tax[k].trim().toLowerCase() == mls[k].trim().toLowerCase()) {
              continue;
            }
        ...
        switch (k) {
          // Don't make remarks for our IDs
          case "bippoId":
          case "obPropId":
          // Don't find the one line address, we already have line1 and line2
          case "oneLine":
            continue;
          default:
            break;
  ...
  clearDataForNewProperty() {
    this.cmaAveragePer = 0;
  }

  getProperty(id, successHandler, errorHandler, requestType = '') {
    let headers = new Headers({"Authorization": "Bearer " + this.authService.getToken()});

    this.clearDataForNewProperty();
    ...
    return this._http.get(environment.API_ENDPOINT + 'property/' + encodeURIComponent(id) + pathSuffix, {headers: headers})
      .subscribe(
        res => {
          let data:any = res.json();
          this.property = data;
          this.propertyCompalatorValue = 0;
          this.areaPropertyGroups = [];
          console.log("Setting property", this.property);

          if ("undefined" === typeof data.market) {
            if (data.hitMergedResponse && data.hitMergedResponse.comps) {
              data.market = {
                arv: 0,
                arvCustom: false,
                asIs: 0,
         ...
          if (this.cmaAveragePer && data.hitMergedResponse && data.hitMergedResponse.building && data.hitMergedResponse.building.size && data.hitMergedResponse.building.size.bldgSize) {
            if (!data.market) {
              data.market = {};
            }

            data.market.cmaAverage = this.cmaAveragePer * data.hitMergedResponse.building.size.bldgSize;
          }

          if (data.hitMergedResponse && data.hitMergedResponse.identifier) {
            if (!data.hitMergedResponse.identifier.bippoId) {
              if (data.mls && data.mls.identifier && data.mls.identifier.bippoId) {
                data.hitMergedResponse.identifier.bippoId = data.mls.identifier.bippoId;
              }
            }
          }
          ...
          if (data.hitMergedResponse && data.tax && data.mls) {
            let remarks = [];
            this.indexReconciledRemarks(data.tax, data.mls, remarks);

            console.log('Setting reconciled remarks', remarks);
            data.hitMergedResponse.reconciledRemarks = remarks;
          }
          ...
          if (this.propertyFlip) {
            ...
            if (this.propertyFlip.sellingClosingCost) {
              this.propertyFlip.sellingClosingCost.titlePolicy = this.getDefaultTitlePolicy(data.hitMergedResponse);
            }
          ...
          let customArv = this.getPropertyFlag('revision_arv');

          if (customArv && customArv.revisionArv && !customArv.revisionArv.isDefault && customArv.revisionArv.finalValue) {
            data.market.arv = +customArv.revisionArv.finalValue;
            data.market.arvCustom = true;
            console.log('Overwrote market.arv value with saved ARV revision: ' + data.market.arv);
          }
  ...
  getAttomId(property: any = undefined) {
    if ("undefined" === typeof property) {
      property = this.property;
    }
    ...
    if (("undefined" !== typeof property.identifier) &&
      property.identifier &&
      ("undefined" !== typeof property.identifier.attomId) &&
      property.identifier.attomId) {
      return property.identifier.attomId;
    }
    return null;
  }
  ...
  getListPrice(property, valueset = undefined) {
    if ("undefined" === typeof property) {
      property = this.property;
    }
    if ("undefined" === typeof valueset) {
  ...
  getClosePrice(property, valueset = undefined) {
    if ("undefined" === typeof property) {
      property = this.property;
    }
    if ("undefined" === typeof valueset) {
  ...
  getClosePricePctOfListPrice(property, valueset = undefined) {
    if ("undefined" === typeof property) {
      property = this.property;
    }
    if ("undefined" === typeof valueset) {
    ...
    if (property) {
      if (("undefined" !== typeof property[valueset]) &&
        ...
        if (("undefined" !== typeof property.mls) &&
            (null !== property.mls)) {
          return numeral(Number(property.mls.closePrice) / Number(property.mls.listPrice) * 100).format("0,0.00");
        } else {
          return "";
        }
    }
  ...
  hasListPrice(property, valueset = undefined) {
    if ("undefined" === typeof property) {
      property = this.property;
    }
    if ("undefined" === typeof valueset) {
  ...
  hasClosePrice(property, valueset = undefined) {
    if ("undefined" === typeof property) {
      property = this.property;
    }
    if ("undefined" === typeof valueset) {
...
//      {'label': 'Wholesale', 'value': 'wholesale'},
  ...
  setupRepairAndRemodel(rr) {
    ...
    } else if (this.simpleQuality) {
      // If using simple quality, clamp value to [1, 3], for Q2-Q4
      if (rr.defaultQuality < 1) {
        rr.defaultQuality = 1;
      } else if (rr.defaultQuality > 3) {
        rr.defaultQuality = 3;
      }
    }
      ...
      .subscribe(
        res => {
          ...
          if ("undefined" === typeof this.propertyFlip.profit) {
            if (("undefined" !== typeof property.mls) && property.mls && property.mls.listPrice) {
              this.propertyFlip.profit = Math.round((property.mls.listPrice * 0.05) / 100) * 100;
            } else if (property.assessment && property.assessment.assessed && property.assessment.assessed.assdTtlValue) {
              this.propertyFlip.profit = Math.round((property.assessment.assessed.assdTtlValue * 0.05) / 100) * 100;
            } else {
          ...
          this.propertyFlip.sellingClosingCost.titlePolicy = this.getDefaultTitlePolicy(property);
  ...
  getDefaultTitlePolicy(property: any) {
    if (property) {
      if (property.assessment && property.assessment.assessed && property.assessment.assessed.assdTtlValue) {
        return Math.floor(property.assessment.assessed.assdTtlValue * 0.01);
      } else if (property.mls) {
        return Math.floor(property.mls.listPrice * 0.01);
      }
    }
  ...
  getQualityDescription(q, stories) {
    ...
    if ("undefined" == typeof q) {
      ...
      switch (q) {
        case 0:
        case 1:
          return '<div><b>Custom Quality</b></div><div>Dwellings with this quality rating are often custom designed for construction on an individual property owner’s site or can be found in high-quality tract developments featuring residences constructed from individual plans or from highly modified or upgraded plans. The design features detailed, high-quality exterior ornamentation, high-quality interior refinements, and detail. The workmanship, materials, and finishes throughout the dwelling are generally of high or very high quality. Amenities are referred to as “luxury” or “upgraded” to reflect their relative expense and, in some cases, rarity.</div><div><br>- Unique structures<br>- Detailed architectural Plans and specs<br>- High quality exterior and interior refinements<br>- Custom designed<br>- Individual Lot or High quality Tract<br>- Plans are modified standard plans or individual plans<br>- Detailed high quality exterior and interior refinements<br>- Higher quality from readily available plans<br>- Significant exterior ornamentation and interiors are well finished<br>- Exceeds acceptable standards</div>';
        case 2:
          return '<div><b>Production Quality</b></div><div>Dwellings with this quality rating meet or exceed the requirements of applicable building codes. Standard or modified standard building plans are utilized and the design includes adequate fenestration and some exterior ornamentation and interior refinements. Materials, workmanship, finish, and equipment are of stock or builder grade and may feature some upgrades.</div><div><br>- Meet or exceed requirements of building codes<br>- Standard or modified standard building plans<br>- Adequate fenestration and some exterior ornamentation and interior refinements<br>- Labor and materials: stock or builder grade</div>';
        case 3:
        case 4:
        case 5:
          return '<div><b>Economy, Basic Quality</b></div><div>Dwellings with this quality rating feature economy of construction and basic functionality as main considerations. Such dwellings feature a plain design using readily available or basic floor plans featuring minimal fenestration and basic finishes with minimal exterior ornamentation and limited interior detail. These dwellings meet minimum building codes and are constructed with inexpensive, stock materials with limited refinements and upgrades.</div><div><br>- Economy construction and basic functionality<br>- Plan design from basic floor plans<br>- Minimal fenestration and basic finishes<br>- Minimal exterior ornamentation and limited interior detail<br>- Inexpensive stock materials and limited refinements and upgrades</div>';
      }
    } else switch (q) {
      case 0:
  ...
  indexAdjustments(comp, key) {
    if (!comp || !comp.adjustment || !comp.adjustment.allAdjustments) {
      return;
    }

    let adjIndex = {};

    for (let a of comp.adjustment.allAdjustments) {
      adjIndex[a.description] = a.adjustmentValue;
      if (key) key[a.description] = 0;
    }

    comp.adjustment.indexedAdjustments = adjIndex;
  }
    ...
    return this._http.get(environment.API_ENDPOINT + 'property/' +
      ...
      .subscribe(
        res => {
          let data:any = res.json();
          let x: any;
          let comps = this.propertyComps = data["comps"];
          let selected = this.propertyCompsSelected = data["defaultComps"];
          let background = this.propertyCompsBackground = data["backgroundComps"];
          let excluded = this.propertyCompsExcluded = data["excludedComps"];

          // Build search index for searchable types of properties
          if (background) for (let i = 0; i < background.length; i++) {
            let bg = background[i];
            bg.gla = 0;
            bg.sale = -1;

            if (bg.mls && bg.mls.building && bg.mls.building.size) {
              bg.gla = bg.mls.building.size.livingSize || bg.mls.building.size.bldgSize || 0;
            }

            if (bg.mls && bg.mls.mls) {
              let close = bg.mls.mls.closeDate || bg.mls.mls.lastChangeDate;

              if (close) {
                bg.sale = Math.floor((Date.now() - +close) / 1000 / 60 / 60 / 24)
              }
            }
          }

          let adjKey = {};
          if (comps) for (let ic of comps) this.indexAdjustments(ic, adjKey);
          if (selected) for (let ic of selected) this.indexAdjustments(ic, adjKey);
          if (background) for (let ic of background) this.indexAdjustments(ic, adjKey);
          if (excluded) for (let ic of excluded) this.indexAdjustments(ic, adjKey);
          data.adjustmentKey = Object.getOwnPropertyNames(adjKey);

          let ratings = data["compsRatings"];
          let defaultRatings = data["defaultCompsRatings"];
          let backgroundRatings = data["backgroundCompsRatings"];
          let excludedRatings = data["excludedCompsRatings"];
          let ratingsEnabled = type && type.toLowerCase() == 'arv';
          data.hasRatings = ratingsEnabled;

          if (!ratingsEnabled || !ratings) {
            ratings = [];
          } else for (let i = 0; i < ratings.length; i++) {
            let r = comps[i].rating = ratings[i];

            switch ([ r.superior.overall, r.equal.overall, r.inferior.overall ].indexOf(Math.max(r.superior.overall, r.equal.overall, r.inferior.overall))) {
              case 0:
                r.best = r.superior;
                r.type = 's'
                break;
          ...
          if (!ratingsEnabled || !defaultRatings) {
            defaultRatings = [];
          } else for (let i = 0; i < defaultRatings.length; i++) {
            let r = selected[i].rating = defaultRatings[i];
          ...
          if (ratingsEnabled && selected) {
            if (selected.length == 3 && defaultRatings.length >= 3) {
              selected[0].rating.best = selected[0].rating.superior;
              selected[0].rating.type = 's';
              selected[1].rating.best = selected[1].rating.equal;
              selected[1].rating.type = 'e';
              selected[2].rating.best = selected[2].rating.inferior;
              selected[2].rating.type = 'i';
            }

            let selectedRatingSum = 0;

            for (let sel of selected) {
              if (sel.rating && sel.rating.best && sel.rating.best.overall) {
                selectedRatingSum += sel.rating.best.overall;
              }
            }

            // Calculate the overall analysis rating as an average of all of the selected comps' ratings
            data.analysisRating = Math.floor(selectedRatingSum / selected.length);
            data.analysisRatingTier = Math.floor(data.analysisRating / 25);
          } else {
            data.analysisRating = data.analysisRatingTier = 0;
          }

          if (!ratingsEnabled || !backgroundRatings) {
            backgroundRatings = [];
          } else for (let i = 0; i < backgroundRatings.length; i++) {
            let r = background[i].rating = backgroundRatings[i];
		...
		r.type = 'i';
          ...
          if (!ratingsEnabled || !excludedRatings) {
            excludedRatings = [];
          } else for (let i = 0; i < excludedRatings.length; i++) {
            let r = excluded[i].rating = excludedRatings[i];
		...
		r.type = 'i';
          ...
          let excludedReasons = data["excludedReasons"];

          if (excludedReasons) {
            for (let i = 0; i < excludedReasons.length; i++) {
              excluded[i].reason = excludedReasons[i];
            }
          }

          this.recacheSelectedComps();
          ...
          if (this.property && !this.property.market) {
            let bldgSize = this.property.hitMergedResponse && this.property.hitMergedResponse.building && this.property.hitMergedResponse.building.size ? this.property.hitMergedResponse.building.size.bldgSize : 0;
            ...
            this.property.market = {
              ...
              rentHigh: 0,
              rentAverage: 0,
              cmaAverage: this.cmaAveragePer ? this.cmaAveragePer * bldgSize : 0
            };
          }

          if (this.property && this.property.market) {
            if (this.propertyCompsMeta.arv && !this.property.market.arvCustom) {
              this.property.market.arv = this.propertyCompsMeta.arv.value;
              console.log('Got ARV: ' + this.property.market.arv);
            }

            if (this.propertyCompsMeta.asIs) {
              this.property.market.asIs = this.propertyCompsMeta.asIs.value;
              console.log('Got As-Is: ' + this.property.market.asIs);
            }

            if (this.propertyCompsMeta.wholesale) {
              this.property.market.wholesale = this.propertyCompsMeta.wholesale.value;
              console.log('Got Wholesale: ' + this.property.market.wholesale);
            }

            if (this.propertyCompsMeta.rent) {
              this.property.market.rentHigh = (isFinite(this.propertyCompsMeta.rent.high)) ? this.propertyCompsMeta.rent.high : 0;
              this.property.market.rentLow = (isFinite(this.propertyCompsMeta.rent.low)) ? this.propertyCompsMeta.rent.low : 0;
              ...
              console.log('Got Rents: ' + this.property.market.rentLow + ' - ' + this.property.market.rentHigh + ' avg ' + this.property.market.rent);
            }
          } else console.log('Cannot sync market values, property missing.');

          console.log("Setting comps", this.propertyCompsMeta, this.propertyComps);

          this.originalCompsSelected = this.propertyCompsSelected;
          this.originalComps = this.propertyComps;

          successHandler();
        },
        error => errorHandler,
        () => console.log('Comps request completed')
      );
  }
...
*/
  recacheSelectedComps() {
    this.propertyCompsSelectedList = [];

    if (this.propertyCompsSelected) {
      for (let x of this.propertyCompsSelected) {
        ...
        .subscribe(
          res => {
            ...
            if (this.areaPropertyGroups) {
                for (let group of this.areaPropertyGroups) {
                ...
                for (let group of this.areaPropertyGroups) {
                  let soldGroup = (group.status == 'Sold') || (group.status == 'Closed');

                  if (soldGroup) {
                    this.cmaAveragePer = group.averages.pricePerSQFT;
                  }

                  for (let prop of group.group) {
                    prop.cmaCompSelected = soldGroup;
                  }
                }

                if (this.property && this.property.market && this.property.hitMergedResponse.building && this.property.hitMergedResponse.building.size) {
                  this.property.market.cmaAverage = this.cmaAveragePer * this.property.hitMergedResponse.building.size.bldgSize;
                  console.log('Got CMA; indexed cmaAverage of ' + this.cmaAveragePer + ' * ' + this.property.hitMergedResponse.building.size.bldgSize);
                } else {
                  console.log('Got CMA; couldn\'t calculate cmaAverage, cmaAveragePer = ' + this.cmaAveragePer + ', property:', this.property);
                }
            }
...
console.log(data);
    ...
    return this._http.get(environment.API_ENDPOINT + 'analytics/' + zip, { headers: headers })
      .subscribe(
        res => {
          ...
          data.active = prices.length > 0 || counts.length > 0;
  ...
  getAttomSchools(attomId, successHandler, errorHandler) {
    this.schoolsData = null;

    let headers = new Headers({ "Authorization": "Bearer " + this.authService.getToken() });
    return this._http.get(environment.API_ENDPOINT + 'schools/' + attomId, { headers: headers })
      .subscribe(
        res => {
          let data = res.json();

          if (data.property && data.property[0]) {
            this.schoolsData = data.property[0];
          }

          successHandler();
        },
        error => errorHandler,
        () => console.log('Schools ' + attomId + ':', this.schoolsData)
      );
  }
...
*/
    ...
    if (!this.propertyFlipData.repairRemodelTotal) {
      this.propertyFlipData.repairRemodelTotal = 0;
    }
...
/*    if (this.propertyFlip.repairAndRemodel.selectedRepairs) {
    ...
    this.propertyFlipData.costToFlip =
        this.propertyFlipData.purchaseClosingCostTotal +
        (this.propertyFlipData.repairRemodelType == 'rrCustom' ? +this.propertyFlipData.repairRemodelCustomTotal : this.propertyFlipData.repairRemodelTotal) +
        this.propertyFlipData.expensesTotal +
        this.propertyFlipData.sellingClosingCostTotal;
  ...
  updatePropertyRentTotals() {
    ...
    if ("other" === this.propertyRent.rent.type) {
      ...
      switch (this.propertyRent.rent.type) {
        case "renta":
          if (this.property && this.property.market && this.property.market.rentHigh) {
            this.propertyRent.rent.value = this.property.market.rentHigh;
          } else if (prop.building && prop.building.size && prop.building.size.livingSize) {
            this.propertyRent.rent.value = Math.floor(prop.building.size.livingSize);
          } else {
          ...
          break;
        case "rentb":
          if (this.property && this.property.market && this.property.market.rentLow) {
            this.propertyRent.rent.value = this.property.market.rentLow;
          } else if (prop.building && prop.building.size && prop.building.size.livingSize) {
            this.propertyRent.rent.value = Math.floor(prop.building.size.livingSize * 2 / 3);
          } else {
    ...
    this.propertyRentData.repairTotal = this.propertyFlipData.repairRemodelTotal;
/*
    for (let item of this.propertyRent.repairs.selectedRepairs) {
    ...
    this.propertyRent.income.net = Math.floor(this.propertyRentData.rentPriceSelectedValue - this.propertyRentData.expensesTotal);

    if (!isFinite(Number(this.propertyRent.income.desiredRoi))) {
      this.propertyRent.income.desiredRoi = 0.1;
    }

    let desiredRoi = Math.max(this.propertyRent.income.desiredRoi, 0.001);
    let investment = this.propertyRentData.repairTotal + this.propertyRentData.purchaseClosingCostSubtotal;
    let annualProfit = this.propertyRent.income.net * 12;

    this.propertyRentData.maxBid = (annualProfit / desiredRoi) - investment;
    ...
    let roiRounded = Math.round(desiredRoi * 100);

    this.propertyRentData.roiFormatted = String(roiRounded) + '%';
//    this.propertyRentData.absRoiFormatted = String(Math.abs(roiRounded)) + '%';

    this.propertyRentData.warnHighBid = this.propertyRentData.maxBid > this.propertyRentData.marketPriceSelectedValue;
  }

  adjustProfitExplicitly(prof: number) {
    this.propertyFlip.profit = prof;
    this.setROI();
  }

  adjustProfitByPct(pct: number) {
    this.propertyFlip.profit =
      Math.floor((this.propertyFlipData.maxBid + this.propertyFlipData.costToFlip) * (pct / 100.0));
    this.setROI();
  }
  ...
  adjustRentRoiToPct(pct: number) {
    this.propertyRent.income.desiredRoi = pct / 100.0;
    this.updatePropertyRentTotals();
  }

  fixRentRoi() {
    this.propertyRent.income.desiredRoi = Math.round(this.propertyRent.income.desiredRoi * 1000) / 1000;
    this.updatePropertyRentTotals();
    this.propertyRentData.absRoiFormatted = String(Math.abs(Math.round(this.propertyRent.income.desiredRoi))) + '%';
  }

/*  adjustCapIncomeByPct(pct: number) {
    this.propertyRent.income.cap = Math.floor(pct) / 100.0;
...
/*    this.propertyRent.repairs[field].included = !this.propertyRent.repairs[field].included;*/
  ...
  getFirstPhoto(property = undefined, valueset = undefined) {
    ...
    return (property
            ...
            && property.resources.photos.urls && property.resources.photos.urls.length > 0) ?
        property.resources.photos.urls[0] : null;
  }

  appendUserPhotos(urls, property = undefined, valueset = undefined) {
    if ("undefined" === typeof property) {
      property = this.property;
    }
    if ("undefined" === typeof valueset) {
    ...
    if (property.resources) {
      if (!property.resources.photos) {
        property.resources.photos = {};
      }

      if (!property.resources.photos.userUrls) {
         property.resources.photos.userUrls = [];
      }

      for (let url of urls) {
        property.resources.photos.userUrls.push(url);
      }

      property.resources.photos.combinedUrls = null;
    }
  }

  getCurrentPhotos() {
    let pr = this.getCurrentProperty();

    if (pr && pr.resources && pr.resources.photos) {
      let photoData = pr.resources.photos;
