...
export class GoogleMapComponent implements OnInit, OnDestroy {
  ...
  private forceNoLabels: boolean = false;
  ...
  @Input()
  set disableLabels(disableLabels: boolean) {
    this.forceNoLabels = disableLabels;
  }
  @Input()
  ...
  set compmeta(meta: any) {
    if (meta && meta.compsFilterUsed) {
      if (meta.compsFilterUsed.proximityInMiles) {
        ...
        this.proximityInBounds = meta.compsFilterUsed.proximityInMiles / 2.0;
...
//        }
  ...
  @Input()
  set fitBounds(enabled: boolean) {
    this.shouldFitBounds = enabled;
  }
  ...
  private proximityInBounds: number = 1.0;
...
//        let dist = this._properties[index].distance;
//        if (dist && dist < this.proximityInBounds) {
        bounds.extend(pos);
//        }
...
//        }
  ...
  getLLFromProperty(property: any) {
    // Check if we have the lat/long given directly on this object
    ...
    // Otherwise, attempt to locate it within the hitMergedResponse or MLS data
    if (property.hitMergedResponse) {
      property = property.hitMergedResponse;
    } else if (property.mls) {
      property = property.mls;
    ...
    if (("undefined" !== typeof property.location) && (null !== property.location) && ("undefined" !== typeof property.location.latitude) && (null !== property.location.latitude)) {
      return new google.maps.LatLng(
    ...
    console.log('Failed to get LL from property:', property);
    return null;
  }
