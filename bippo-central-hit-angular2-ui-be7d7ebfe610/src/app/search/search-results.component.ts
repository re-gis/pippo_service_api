...
export class SearchResultsComponent implements OnInit, AfterViewInit, OnChanges {
    ...
    ngOnInit() {
      ...
      if (this.searchService.backToSearch) {
        this.page = this.searchService.backToSearchPage || 0;
      } else {
        this.searchService.backToSearchPage = this.page = (("undefined" !== typeof this.searchQuery) && (this.searchQuery) && ("undefined" !== typeof this.searchQuery["page"])) ? this.searchQuery["page"] : 0;
      }
      ...
      this.searchService.backToSearchUrl = this.router.url;
    }
  ...
  isSearchReady() {
    ...
    search() {
      ...
      this.searchService.backToSearchPage = this.page;

      if ("undefined" !== typeof this.searchQuery["searchid"]) {
        console.log("getSavedSearch('" + this.searchQuery["searchid"] + "')");
        this.searchService.getSavedSearchPage(this.searchQuery["searchid"], this.searchQuery["page"], rawData => {
          if (rawData && rawData["content"] && rawData["content"].length > 0) {
            this.searchService.setMLS(rawData["content"][0].mls.mlsRegion);
          }
        ...
        this.searchService.getAdvanceSearchPage(this.searchQuery, this.searchQuery["page"], rawData => {
          if (rawData && rawData["content"] && rawData["content"].length > 0) {
            this.searchService.setMLS(rawData["content"][0].mls.mlsRegion);
          }
        }, (error:any, caught: Observable<any>) => {
    ...
    viewPropertyByIndex(index: number) {
    ...
    getPhotoDisplayType(prop) {
      if (this.searchService.hasLargePhotos(prop)) {
        return 'LargePhoto';
      } else if (this.searchService.hasFrontPhoto(prop)) {
        return 'Photo';
      }
