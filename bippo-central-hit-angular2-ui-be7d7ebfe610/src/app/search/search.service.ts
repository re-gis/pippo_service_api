...
export class SearchService {
  ...
  watchlistNeedsRefresh: boolean = false;
  ignorelistArray: Array<string>;
  craigslistPage: any = {};
  craigslistPageContent: any = [];
  craigslistPageMax: number = 1;
  craigslistMeta: Array<any> = [];
  ...
  backToSearchPage: number = 0;
  backToSearchUrl: string;
  lastLoadedPage: number = 0;
  viewedPropertyIndex: number = -1;
  relevantMlsContext: string = '';
  ...
  hasPreviousProperty() {
    return this.viewedPropertyIndex >= 0 && (this.viewedPropertyIndex > 0 || this.lastLoadedPage > 0);
  }

  hasNextProperty() {
    return this.viewedPropertyIndex >= 0 && (!this.meta || !this.meta.totalPages || this.lastLoadedPage < this.meta.totalPages) && (this.viewedPropertyIndex + 1 >= this.size || this.viewedPropertyIndex + 1 < this.properties.length);
  }

  viewProperty(index, router) {
    if (!this.properties[index]) {
      return;
    }

    this.viewedPropertyIndex = index;
    this.backToSearchPage = this.lastLoadedPage + 1;

    router.navigate(['/property', this.getPropertyId(this.properties[index])]);
  }

  viewNextProperty(router) {
    if (this.viewedPropertyIndex < 0) {
      return false;
    } else if (this.viewedPropertyIndex < this.size - 1) {
      if (this.viewedPropertyIndex + 1 >= this.properties.length) {
        return false;
      }

      this.viewedPropertyIndex++;
      router.navigate(['/property', this.getPropertyId(this.properties[this.viewedPropertyIndex])]);
      return true;
    } //else if (this.lastLoadedPage > maxPage)

    this.getAdvanceSearchPage(null, this.lastLoadedPage + 1, data => {
      this.viewProperty(0, router);
  ...
  viewPreviousProperty(router) {
    if (this.viewedPropertyIndex < 0) {
      return false;
    } else if (this.viewedPropertyIndex > 0) {
      this.viewedPropertyIndex--;
      router.navigate(['/property', this.getPropertyId(this.properties[this.viewedPropertyIndex])]);
      return;
    } else if (this.lastLoadedPage < 2) {
      // There is no previous page to go to
      return false;
    }

    this.getAdvanceSearchPage(null, this.lastLoadedPage - 1, data => {
      this.viewProperty(this.size - 1, router);
    }, (error:any, caught: Observable<any>) => {
      console.log(error);
      return caught;
    });
        ...
        .subscribe(
          res => {
            ...
            for (let x of this.searchMetadata.propertyTypes) {
              ...
              if (-1 !== ["Residential"].indexOf(x.value)) {
  ...
  getMlsRegions() {
    this.mlsRegions = [
      { value: "NTREIS", label: "NTREIS: Dallas - Ft. Worth" },
      { value: "HAR", label: "HAR: Houston" }
    ];
  }
  ...
  setMLS(abbr) {
    if (abbr) {
      let description = abbr;
      this.getMlsRegions();
      this.mlsRegions.forEach((val, i, arr) => {
        if (val.value == abbr) {
          description = val.label;
        }
      });
      this.relevantMlsContext = description;
    } else {
      this.relevantMlsContext = '';
    }
  }

  selectMlsRegion(r) {
    this.search.mlsRegion = r;
  }
  ...
  setStatusDateFrom(e) {
    this.search.statusChangeDateFrom = e;
    this.search.statusChangeTimestampFrom = e + 'T00:00:00.000';
  }

  setStatusDateTo(e) {
    this.search.statusChangeDateTo = e;
    this.search.statusChangeTimestampTo = e + 'T00:00:00.000';
  }
  ...
  getFirstLargePhoto(property) {
    ...
    return (("undefined" !== typeof property.resources)
            ...
            && (property.resources.photos.largePhotoUrls.length > 0)) ?
        property.resources.photos.largePhotoUrls[0] : null;
  }

  hasLargePhotos(property) {
    if (property && ("undefined" !== typeof property.bippoProperty)) {
      property = property.bippoProperty;
    }
    return (("undefined" !== typeof property.resources)
  ...
  getSimpleSearch(query, page, successHandler, errorHandler) {
    let headers = new Headers({"Authorization": "Bearer " + this.authService.getToken()});
  ...
  getCraigslistPage(region, page, successHandler, errorHandler) {
    let headers = new Headers({"Authorization": "Bearer " + this.authService.getToken()});

    this.searchPending = true;

    return this._http.get(environment.API_ENDPOINT + 'extd/cl/page?region=' + region + '&page=' + String(page) + '&count=20', { headers: headers })
      .subscribe(
        res => {
          this.craigslistPage = res.json();
          this.craigslistPageContent = this.craigslistPage.content || [];
          this.craigslistPageMax = this.craigslistPage.totalPages || 1;
  ...
  setAdvanceSearchQuery(query) {
    // Do this so we can filter out fields we don't need to search on
    let data = {};
    this.size = 20;
    this.page = 0;
    this.searchName = "";
  ...
  getAdvanceSearchPage(query, usePage, successHandler, errorHandler) {
    // Allow passing a null query- this will attempt to re-search the previous search with a new page number
    if (query) {
    ...
    let url = environment.API_ENDPOINT + 'search/advanced?page=' + usePage + '&size=' + String(this.size) + '&searchName=' + String(this.searchName);
    this.lastLoadedPage = usePage;
  ...
  getAdvanceSearch(query, successHandler, errorHandler) {
    this.setAdvanceSearchQuery(query);
    this.renderSearch();
    let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + this.authService.getToken() });
    let options = new RequestOptions({ headers: headers });
    let url = environment.API_ENDPOINT + 'search/advanced?page=' + String(this.page) + '&size=' + String(this.size) + '&searchName=' + String(this.searchName);
  ...
  getSavedSearchPage(id: string, usePage, successHandler, errorHandler) {
    let headers = new Headers({"Authorization": "Bearer " + this.authService.getToken()});
    return this._http.get(environment.API_ENDPOINT + 'saved_search/id/'+id, {headers: headers})
      .subscribe(
        res => {
          ...
          this.getAdvanceSearchPage(this.savedProperty.searchFilter, usePage, successHandler, errorHandler);
        },
        error => errorHandler,
    ...
    return this._http.get(environment.API_ENDPOINT + 'saved_search/id/'+id, {headers: headers})
      .subscribe(
        res => {
          ...
          this.search.update(query);
          console.log(this.search);
          this.getAdvanceSearch(this.savedProperty.searchFilter, null, errorHandler);
        },
        error => errorHandler,
        () => console.log(this.savedProperty)
      );
    ...
    return this._http.get(environment.API_ENDPOINT + 'watchlist/full?page='+
      ...
      .subscribe(
        res => {
          ...
          this.watchlistNeedsRefresh = false;
          return data;
        },
        error => errorHandler
      );
  ...
  triggerWatchlistClientSide(prop: any) {
    let headers = new Headers({"Authorization": "Bearer " + this.authService.getToken()});
    let options = new RequestOptions({ headers: headers });
    let id: string;
    ...
    if (-1 !== this.watchlistArray.indexOf(id)) {
      // Already watched
      return;
    }
    ...
    if ("string" !== typeof prop) {
      ...
      // TODO Properly address sources that add a watchlist listing without providing property data,
      // as this causes the entire watchlist to have to be re-cached since the data will be missing
//      this.watchlistProperties.push({});
        this.watchlistNeedsRefresh = true;
    }
  }
  ...
  toggleWatchlistSearchListing(prop: any, errorHandler) {
    ...
    if (-1 !== pos) {
      ...
      for (pos=0; pos<this.watchlistProperties.length; pos++) {
        if (this.watchlistProperties[pos] && this.watchlistProperties[pos].bippoProperty && id === this.watchlistProperties[pos].bippoProperty.identifier.bippoId) {
...
//        this.watchlistProperties.push({});
  ...
  getNewProperties(region, successHandler, errorHandler) {
    let headers = new Headers({
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + this.authService.getToken()
    });
    let options = new RequestOptions({ headers: headers });
    let newListingsSearch = this.getDefaultSearch(region);
  ...
  getCraigslistMetadata() {
    let headers = new Headers({ 'Authorization': 'Bearer ' + this.authService.getToken() });

    return this._http.get(environment.API_ENDPOINT + 'extd/cl/regions', { headers: headers })
      .subscribe(
        res => {
          let data:any = res.json();

          if (data["regions"]) {
            this.craigslistMeta = data["regions"];
          } else {
            console.log("bad craigslist metadata response", data);
          }
  ...
  getPropertyId(property: any, encode: boolean = false) {
    if (property) {
      if ("undefined" !== typeof property.bippoProperty) {
        property = property.bippoProperty;
      }
      ...
      if (property.identifier) {
        let i = property.identifier;

        if (i.bippoId) {
          return encode ? encodeURIComponent(i.bippoId) : i.bippoId;
        } else if (i.mlsNumber) {
          return encode ? encodeURIComponent(i.mlsNumber) : i.mlsNumber;
        } else if (i.obPropId) {
          // Reconsider using this at all?
          return encode ? encodeURIComponent(i.obPropId) : i.obPropId;
        }
      }
